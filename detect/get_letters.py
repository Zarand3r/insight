...
import classifier.CNN.emnist_cnn as clf
...
def main():
	...
	try:
		...
		for (im, box, _, _) in boxes:
			...
			cv2.imwrite("output/letters" + str(index) + ".png", border)
...
def convert_handwritten_image_to_emnist_format():
	handwritten_dataset = os.listdir("output/letters")
	num_images = len(handwritten_dataset)

	for file in os.listdir("output/letters"):
		if file.startswith('.'):
	...
	for file in os.listdir("output/letters"):
		if not file.startswith('.'):
			img = Image.open("output/letters"+file)
			arr = np.array(img)
			try:
...
def classify(data):
	with tf.Session() as sess:
		checkpoint = tf.train.latest_checkpoint('../classifier/models/emnist_model/')
		saver = tf.train.import_meta_graph("../classifier/models/emnist_model/model.ckpt-20000.meta")
		...
		emnist_classifier = tf.estimator.Estimator(
			...
			model_dir="../classifier/models/emnist_model")
